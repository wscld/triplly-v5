...
// Helper: Check access
async function checkTodoAccess(userId: string, todoIdStr: string | null, travelIdStr: string | null, minRole: 'viewer' | 'editor') {
    ...
    if (todoIdStr) {
        ...
        if (!todo) return { error: 'Todo not found', status: 404 as const };
        travelId = todo.travelId;
    }

    if (!travelId) return { error: 'Invalid request', status: 400 as const };

    const member = await memberRepo.findOne({ where: { travelId, userId } });
    if (!member) return { error: 'Not a member of this travel', status: 403 as const };
...
todos.get('/', async (c) => {
    ...
    if (!travelId) return c.json({ error: 'Missing travelId' }, 400);
...
todos.patch('/:id', zValidator('json', updateTodoSchema), async (c) => {
    ...
    const { todo } = result;
    if (!todo) return c.json({ error: 'Todo not found' }, 404); // Should be covered by checkTodoAccess
