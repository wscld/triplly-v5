 ...
 */
export function requireTravelAccess(minRole: RequiredRole) {
    return async (c: Context, next: Next) => {
        ...
        const requiredLevel = roleHierarchy[minRole as MemberRole];
...
export function getMember(c: Context): TravelMember {
    return c.get('member');
}
