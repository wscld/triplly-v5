...
async function checkTravelAccess(userId, travelId, minRole) {
...
async function checkActivityAccess(userId, activityId, minRole) {
...
function calculateNewIndex(before, after) {
...
activities.patch('/:activityId', zValidator('json', updateActivitySchema), async (c) => {
    ...
    if (data.googlePlaceId !== undefined)
        activity.googlePlaceId = data.googlePlaceId;
    await activityRepo.save(activity);
    return c.json(activity);
});
